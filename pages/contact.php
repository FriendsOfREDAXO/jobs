...
if (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply')) {
    ...
    $contact = new FriendsOfRedaxo\Jobs\Contact($form['contact_id']);
...
if (1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT) || 'delete' === $func) {
    ...
    $contact = new FriendsOfRedaxo\Jobs\Contact($contact_id);
...
?>
	<form action="<?= rex_url::currentBackendPage() ?>" method="post">
		<div class="panel panel-edit">
			<header class="panel-heading"><div class="panel-title"><?= rex_i18n::msg('jobs_contact') ?></div></header>
			<div class="panel-body">
				<input type="hidden" name="form[contact_id]" value="<?= $entry_id ?>">
				<fieldset>
					<legend><?= rex_i18n::msg('jobs_contact') ?></legend>
					<div class="panel-body-wrapper slide">
						<?php
                            $contact = new FriendsOfRedaxo\Jobs\Contact($entry_id);
                            $readonly = true;
                            ...
                            \TobiasKrais\D2UHelper\BackendHelper::form_input('jobs_email', 'form[email]', $contact->email, true, $readonly, 'email');
                            \TobiasKrais\D2UHelper\BackendHelper::form_input('jobs_phone', 'form[phone]', $contact->phone, true, $readonly);
                            \TobiasKrais\D2UHelper\BackendHelper::form_input('jobs_phone_video', 'form[phone_video]', $contact->phone_video, false, $readonly);
                            \TobiasKrais\D2UHelper\BackendHelper::form_infotext('jobs_phone_video_hint', 'phone_video_label');
...
if ('' === $func) {
    $query = 'SELECT contact_id, name, email '
        . 'FROM '. rex::getTablePrefix() .'jobs_contacts';
    ...
    $list->setColumnLabel('email', rex_i18n::msg('jobs_email'));
    ...
    $list->setNoRowsMessage(rex_i18n::msg('jobs_no_contacts_found'));

    $fragment = new rex_fragment();
    $fragment->setVar('title', rex_i18n::msg('jobs_contacts'), false);
