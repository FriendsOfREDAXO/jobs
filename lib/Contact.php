...
class Contact
{
    ...
    {
        if ($contact_id > 0) {
            $query = 'SELECT * FROM '. rex::getTablePrefix() .'jobs_contacts '
    ...
    {
        $query = 'DELETE FROM '. rex::getTablePrefix() .'jobs_contacts '
    ...
    {
        $query = 'SELECT contact_id FROM '. rex::getTablePrefix() .'jobs_contacts ORDER BY name';
        $result = rex_sql::factory();
        $result->setQuery($query);
