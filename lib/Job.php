 ...
 * Job details.
 */
class Job implements \TobiasKrais\D2UHelper\ITranslationHelper
{
    ...
    public function __construct($job_id, $clang_id)
    {
        ...
        if ($job_id > 0) {
            $query = 'SELECT * FROM '. rex::getTablePrefix() .'jobs_jobs AS jobs '
    ...
    public function changeStatus(): void
    {
        if ('online' === $this->online_status) {
            if ($this->job_id > 0) {
                $query = 'UPDATE '. rex::getTablePrefix() .'jobs_jobs '
    ...
    {
        $query_lang = 'DELETE FROM '. rex::getTablePrefix() .'jobs_jobs_lang '
            .'WHERE job_id = '. $this->job_id
        ...
        $query_main = 'SELECT * FROM '. rex::getTablePrefix() .'jobs_jobs_lang '
            .'WHERE job_id = '. $this->job_id;
        $result_main = rex_sql::factory();
        $result_main->setQuery($query_main);
        if (0 === $result_main->getRows()) {
            $query = 'DELETE FROM '. rex::getTablePrefix() .'jobs_jobs '
                .'WHERE job_id = '. $this->job_id;
            $result = rex_sql::factory();
            $result->setQuery($query);
        }
    ...
    {
        $query = 'SELECT country_code FROM '. rex::getTablePrefix() .'jobs_jobs ';
        if ($ignore_offline) {
    ...
    {
        $query = 'SELECT city FROM '. rex::getTablePrefix() .'jobs_jobs_lang AS lang '
            .'LEFT JOIN '. rex::getTablePrefix() .'jobs_jobs AS jobs '
...
//				.'"validThrough" : "'. date( "Y-m-d", strtotime( $this->date. " +2 month" ) ) .'T00:00",'. PHP_EOL
                ...
                .'"hiringOrganization" : {'. PHP_EOL
                    ...
                    .'"name" : '. json_encode((string) rex_config::get('jobs', 'company_name', ''), JSON_UNESCAPED_UNICODE) .','. PHP_EOL
                    .'"sameAs" : "'. (rex_addon::get('yrewrite')->isAvailable() ? rex_yrewrite::getCurrentDomain()->getUrl() : rex::getServer()) .'",'. PHP_EOL
                    .'"logo" : "'. rtrim(rex_addon::get('yrewrite')->isAvailable() ? rex_yrewrite::getCurrentDomain()->getUrl() : rex::getServer(), '/') . rex_url::media((string) rex_config::get('jobs', 'logo', '')) .'"'. PHP_EOL
                .'},'. PHP_EOL
                .'"jobLocation": {'. PHP_EOL
                    .'"@type": "Place",'. PHP_EOL
                    .'"address": {'. PHP_EOL
...
//						.'"streetAddress": "1600 Amphitheatre Pkwy, ",'. PHP_EOL
    ...
    {
        $query = 'SELECT job_id FROM '. rex::getTablePrefix() .'jobs_jobs '
                .'WHERE hr4you_job_id = '. $hr4you_id;
        $result = rex_sql::factory();
        $result->setQuery($query);

        if ($result->getRows() > 0) {
            return new self((int) $result->getValue('job_id'), (int) rex_config::get('jobs', 'hr4you_default_lang'));
        }
        return false;
    }
    ...
    {
        $query = 'SELECT job_id, hr4you_job_id FROM '. rex::getTablePrefix() .'jobs_jobs '
        ...
        $jobs = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $jobs[$result->getValue('hr4you_job_id')] = new self((int) $result->getValue('job_id'), (int) rex_config::get('jobs', 'hr4you_default_lang'));
            $result->next();
        }
    ...
    {
        $query = 'SELECT job_id FROM '. rex::getTablePrefix() .'jobs_jobs_lang '
                .'WHERE clang_id = '. $clang_id ." AND translation_needs_update = 'yes' "
                .'ORDER BY name';
        if ('missing' === $type) {
            $query = 'SELECT main.job_id FROM '. rex::getTablePrefix() .'jobs_jobs AS main '
                    .'LEFT JOIN '. rex::getTablePrefix() .'jobs_jobs_lang AS target_lang '
                        .'ON main.job_id = target_lang.job_id AND target_lang.clang_id = '. $clang_id .' '
                    .'LEFT JOIN '. rex::getTablePrefix() .'jobs_jobs_lang AS default_lang '
        ...
        for ($i = 0; $i < $result->getRows(); ++$i) {
            ...
            if (0 === $job->job_id && (bool) rex_config::get('jobs', 'use_hr4you')) {
                $job = new self((int) $result->getValue('job_id'), (int) rex_config::get('jobs', 'hr4you_default_lang'));
            }
            if (0 === $job->job_id) {
